	...
	return (
		<div className='flex flex-col gap-2'>
			<div className='relative flex items-stretch justify-between gap-2 rounded-md bg-gradient-to-br from-indigo-800 to-blue-700 p-2 font-semibold text-white'>
				{auth.role === 'admin' ||
				!isPast(new Date().setDate(selectedDate.getDate() - 1)) ? (
					<button
						title='Go to yesterday'
						className={
							'rounded hover:bg-gradient-to-br hover:from-indigo-600 hover:to-blue-600'
						}
				...
				{isEditing ? (
					...
					>
						<input
							...
							type='Date'
							min={
								auth.role !== 'admin' && new Date().toLocaleDateString('en-CA')
							}
							required
							autoFocus
