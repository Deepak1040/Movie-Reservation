...
const User = () => {
	...
	const onUpdateUser = async (data) => {
		try {
			...
			fetchUsers();
			toast.success(
				`Update ${response.data.data.username} to ${response.data.data.role} successful!`,
				{
					position: 'top-center',
					autoClose: 2000,
					pauseOnHover: false,
	...
	const handleDelete = (data) => {
		const confirmed = window.confirm(
			`Do you want to delete user ${data.username}?`
		);
		if (confirmed) {
			onDeleteUser(data);
		}
		...
		<div className='flex min-h-screen flex-col gap-4 bg-gradient-to-br from-indigo-900 to-blue-500 pb-8 text-gray-900 sm:gap-8'>
			...
			<div className='mx-4 flex h-fit flex-col gap-2 rounded-lg bg-gradient-to-br from-indigo-200 to-blue-100 p-4 drop-shadow-xl sm:mx-8 sm:p-6'>
				...
				<div
					...
					style={{
						gridTemplateColumns:
							'repeat(3, minmax(max-content, 1fr)) max-content max-content',
					}}
				>
									...
									<div className='flex gap-2 border-t-2 border-indigo-200 px-2 py-1'>
										{user.role === 'user' && (
											<button
												...
												onClick={() =>
													onUpdateUser({ id: user._id, role: 'admin' })
												}
												disabled={isUpdating}
											>
												Set Admin
										...
										{user.role === 'admin' && (
											<button
												className='flex w-[115px] items-center justify-center gap-1 rounded bg-gradient-to-r from-indigo-600 to-blue-500 py-1 pl-2 pr-1.5 text-sm font-medium text-white hover:from-indigo-500 hover:to-blue-400 disabled:from-slate-500 disabled:to-slate-400'
												onClick={() =>
													onUpdateUser({ id: user._id, role: 'user' })
												}
												disabled={isUpdating}
											>
												Set User
										...
										<button
											...
											onClick={() =>
												handleDelete({ id: user._id, username: user.username })
											}
											disabled={isDeleting}
										>
											DELETE
				...
				{ticketsUser && (
					<>
						<h2 className='mt-4 text-2xl font-bold text-gray-900'>
							Viewing {ticketsUser}'s tickets
						</h2>
						{tickets.length === 0 ? (
							<p className='text-center'>
								This user have not purchased any tickets yet
							</p>
						) : (
