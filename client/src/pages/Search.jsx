...
const Search = () => {
	...
	const [isDeletingCheckedShowtimes, setIsDeletingCheckedShowtimes] =
		useState(false);
	const [deletedCheckedShowtimes, setDeletedCheckedShowtimes] = useState(0);
	const [isReleasingCheckedShowtimes, setIsReleasingCheckedShowtimes] =
		useState(false);
	const [releasedCheckedShowtimes, setReleasedCheckedShowtimes] = useState(0);
	const [isUnreleasingCheckedShowtimes, setIsUnreleasingCheckedShowtimes] =
		useState(false);
	const [unreleasedCheckedShowtimes, setUnreleasedCheckedShowtimes] =
		useState(0);
	...
	const filteredShowtimes = showtimes
		.filter((showtime) => {
			...
			return (
				(!filterCinema ||
					filterCinema
						.map((cinema) => cinema.value)
						.includes(showtime.theater.cinema._id)) &&
				(!filterTheater ||
					filterTheater
						.map((theater) => theater.value)
						.includes(showtime.theater.number)) &&
				(!filterMovie ||
					filterMovie
						.map((movie) => movie.value)
						.includes(showtime.movie._id)) &&
				(!filterDate ||
					filterDate
						.map((showtime) => showtime.value)
						.includes(formattedDate)) &&
				(!filterDateFrom ||
					new Date(filterDateFrom.value) <= new Date(formattedDate)) &&
				(!filterDateTo ||
					new Date(filterDateTo.value) >= new Date(formattedDate)) &&
				(!filterPastDate ||
					new Date(
		...
		.sort((a, b) => {
			if (sortCinema) {
				return (
					sortCinema *
					a.theater.cinema.name.localeCompare(b.theater.cinema.name)
				);
			}
			if (sortTheater) {
			...
			if (sortTime) {
				return (
					...
					(new Date(a.showtime)
						...
						.padStart(2, '0')
						.concat(
							new Date(a.showtime).getMinutes().toString().padStart(2, '0')
						) -
						new Date(b.showtime)
							.getHours()
							.toString()
							.padStart(2, '0')
							.concat(
								new Date(b.showtime).getMinutes().toString().padStart(2, '0')
							))
				);
			}
			if (sortBooked) {
	...
	const handleReleaseCheckedShowtimes = () => {
		const confirmed = window.confirm(
			`Do you want to release ${checkedShowtimes.length} checked showtimes?`
		);
		if (confirmed) {
	...
	const handleUnreleasedCheckedShowtimes = () => {
		const confirmed = window.confirm(
			`Do you want to unreleased ${checkedShowtimes.length} checked showtimes?`
		);
		if (confirmed) {
	...
	const onUnreleasedCheckedShowtimes = async () => {
		...
		await Promise.all(releasePromises);
		toast.success(
			`Unreleased ${successCounter} checked showtimes successful!`,
			{
				position: 'top-center',
				autoClose: 2000,
				pauseOnHover: false,
		...
		<div className='flex min-h-screen flex-col gap-4 bg-gradient-to-br from-indigo-900 to-blue-500 pb-8 text-gray-900 sm:gap-8'>
			...
			<div className='mx-4 flex h-fit flex-col gap-2 rounded-lg bg-gradient-to-br from-indigo-200 to-blue-100 p-4 drop-shadow-xl sm:mx-8 sm:p-6'>
					...
					{isOpenFilter && (
						<div className=''>
							<div className='flex flex-col'>
								<h4 className='pt-1 text-lg font-bold text-gray-800'>
									Cinema :
								</h4>
								<Select
									value={filterCinema}
									options={Array.from(
										new Set(
											showtimes.map((showtime) => showtime.theater.cinema._id)
										)
									).map((value) => ({
										value,
										label: showtimes.find(
											(showtime) => showtime.theater.cinema._id === value
										).theater.cinema.name,
									}))}
									onChange={(value) => {
							...
							<div className='flex flex-col'>
								...
								<Select
									...
									options={Array.from(
										new Set(
											showtimes.map((showtime) => showtime.theater.number)
										)
									)
										.sort((a, b) => a - b)
										.map((value) => ({
							...
							<div className='flex flex-col'>
								...
								<Select
									value={filterMovie}
									options={Array.from(
										new Set(showtimes.map((showtime) => showtime.movie._id))
									).map((value) => ({
										value,
										label: showtimes.find(
											(showtime) => showtime.movie._id === value
										).movie.name,
									}))}
									onChange={(value) => {
				...
				<div className='flex items-end'>
					...
					{isFetchingShowtimesDone && (
						<div className='ml-auto flex items-center gap-1 px-1 text-sm font-medium'>
							<InformationCircleIcon className='h-5 w-5' /> Showing{' '}
							{filteredShowtimes.length} filtered showtimes
						</div>
					)}
				</div>
				...
				<div
					...
					style={{
						gridTemplateColumns:
							'34px repeat(7, minmax(max-content, 1fr)) 104px',
					}}
				>
					...
					>
						...
						{sortCinema === -1 && (
							<ChevronDownIcon className='ml-auto w-6 h-6' />
						)}
					</button>
					<button
						className='sticky top-0 flex justify-center bg-gradient-to-br from-gray-800 to-gray-700 hover:from-gray-700 hover:to-gray-600 px-2 py-1 text-center text-xl font-semibold text-white'
